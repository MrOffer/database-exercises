USE music_db;
INSERT INTO songs (artist, song_title, release_date, length, genre)
values('Michael jackson', 'Thriller', 1982, 47.3, 'Pop'),
      ('AC/DC', 'Back in Black', 1980, 26.4, 'Hard Rock'),
      ('Meat Loaf', 'Bat Out Of Hell', 1977, 21.7, 'Hard Rock'),
      ('Pink Floyd', 'The Dark Side of the Moon', 1973,	24.2, 'Progressive Rock'),
      ('Whitney Houston', 'The Bodyguard', 1992, 28.4, 'R&B'),
      ('Eagles', 'Their Greatest Hits', 1976, 41.2, 'Soft Rock'),
      ('Bee Gees', 'Saturday Night Fever', 1977, 21.6, 'Disco'),
      ('Fleetwood Mac', 'Rumours', 1977, 27.9, 'Soft Rock'),
      ('Shania Twain', 'Come On Over', 1997, 29.6, 'Country'),
      ('Various Artists', 'Grease: The Original Soundtrack from the Motion Picture', 1978, 14.4, 'Rock And Roll'),
      ('Led Zeppelin', 'Led Zeppelin IV', 1971, 29.0, 'Hard Rock'),
      ('Michael Jackson', 'Bad', 1987, 22.2, 'Pop'),
      ('Alanis Morissette', 'Jagged Little Pill', 1995, 24.4, 'Alternative Rock'),
      ('Celine Diopn', 'Falling Into You', 1996, 20.2, 'Pop'),
      ('Eagles', 'Hotel California', 1976, 31.5, 'Soft Rock'),
      ('The Beatles', 'Sgt. Pepper''s LonelyHearts Club Band', 1967, 18.2, 'Rock'),
      ('Various Artist', 'Dirty Dancing', 1987, 17.9, 'Pop'),
      ('Michael Jackson', 'Dangerous', 1991, 17.3, 'Pop'),
      ('Celine Dion', 'Let''s Talk About Love', 1997, 19.3, 'Pop'),
      ('the Beatles', '1', 2000, 23.4, 'Rock'),
      ('Abba', 'Gold: Greatest Hits', 1992, 23.0, 'Pop'),
      ('Madonna', 'The Immaculate Collection', 1990, 19.4, 'Pop'),
      ('The Beatles', 'Abbey Road', 1969, 14.4, 'Rock'),
      ('Shania Twain', 'Come On Over', 1997, 29.6, 'Country'),
      ('Bruce Springsteen', 'Born In The U.S.A.', 1984, 19.6, 'Rock'),
      ('Dire Straits', 'Brother In Arms', 1985, 17.7, 'Soft Rock'),
      ('James Horner', 'Titanic', 1997, 18.1, 'Pop'),
      ('Metallica', 'Metallica', 1991, 25.2, 'Heavy Metal'),
      ('Nirvana', 'Nevermind', 1991, 16.7, 'Grunge'),
      ('Pink Floyd', 'The Wall', 1979, 18.7, 'Progressive Rock'),
      ('Santana', 'Supernatural', 1999, 20.5, 'Latin rock'),
      ('Guns N'' Roses', 'Appetite for Destruction', 1987, 21.9, 'Hard Rock');



# Pick a given song title. What is the artist of this song title?
#
# select artist from songs where song_title = "1";
#
#   What are the three longest songs?
#
# select * from songs order by length DESC Limit 3;
#
#   How many songs are there in the table?
#
# SELECT COUNT(*) FROM songs;
#
#   What artist has the most songs?
#
# SELECT DISTINCT artist, COUNT(*) FROM songs
# GROUP BY artist
# ORDER BY COUNT(*) DESC LIMIT 1;